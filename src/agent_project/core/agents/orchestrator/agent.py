...
class OrchestratorAgent(BaseAgent):
    ...
    def _build_graph(self) -> StateGraph:
        ...
        graph.add_conditional_edges(
            ...
            {"specialist": "route_to_specialist", "general": "handle_general_query"},
        )
    ...
    async def _route_to_specialist(self, state: OrchestratorState) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "Routing to specialist", specialist=specialist, intent=state.intent
            )
            ...
            result = await specialist_agent.process_query(
                query=state.query, session_id=state.session_id, user_id=state.user_id
            )
    ...
    async def _handle_general_query(self, state: OrchestratorState) -> Dict[str, Any]:
        ...
        try:
            ...
            call_result = vector_client.similarity_search(
                query=state.query, limit=5, similarity_threshold=0.7
            )
            results = (
                await call_result if hasattr(call_result, "__await__") else call_result
            )
    ...
    async def _get_specialist_agent(self, specialist_type: str):
        ...
        try:
            ...
            logger.error(
                "Failed to load specialist agent",
    ...
    async def process_query(
        ...
        try:
            ...
            specialist_val = (
                result.get("specialist_agent")
                if hasattr(result, "get")
                else getattr(result, "specialist_agent", None)
            )
