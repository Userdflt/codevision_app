...
from agent_project.core.agents.orchestrator.agent import OrchestratorAgent, OrchestratorState
...
class TestOrchestratorAgent:
    ...
    @pytest.mark.asyncio
    async def test_process_query_success(self, orchestrator, mock_vector_client, mock_llm_client):
        ...
        with patch.object(orchestrator, 'intent_classifier') as mock_classifier:
            ...
            with patch.object(orchestrator, '_get_specialist_agent') as mock_get_agent:
                ...
                assert result["response"] == "The minimum R-value for Zone 3 walls is R2.8."
    ...
    async def test_general_query_handling(self, orchestrator, mock_vector_client):
        ...
        with patch.object(orchestrator, 'intent_classifier') as mock_classifier:
            ...
            with patch.object(orchestrator, 'vector_client', mock_vector_client):
                with patch.object(orchestrator, '_generate_general_response') as mock_generate:
                    mock_generate.return_value = "General response about building codes."
